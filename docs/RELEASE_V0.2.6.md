...
# v0.2.6 Linux distribution release checklist

`v0.2.6` supersedes the immutable, unpublished `v0.2.5` tag. It targets only
Snap Store, AppImageHub, and AUR.

## Repository gates

1. All tag-verified version fields, Snap metadata, and AppImageHub AppStream
   metadata must be `0.2.6`.
...
3. The canonical catalog asset must be named
   `ImgConvert-0.2.6-x86_64.AppImage` and coexist with the signed updater asset.
