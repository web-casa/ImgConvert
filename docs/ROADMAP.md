...
- [x] **文件访问抽象尖刺(新增,前移)**:新增 `src-tauri/src/access.rs` 授权路径边界,导入扫描、输出目录和剪贴板临时文件都先收口为 grant;不依赖 canonical 路径,为 Flatpak portal 映射路径和 macOS security-scoped bookmark shim 留接口。运行时策略会让 AppImage 继续使用宿主 `zenity`/`kdialog` workaround，而 Flatpak 和严格 Snap 使用 GTK portal picker；这些 portal 沙盒首次转换必须选择输出目录以取得可写 document-portal scope。交互式 portal 选择/授权流仍作为人工发布验收项。
...
- [x] **(macOS 阶段)输出目录会话授权闭环**:macOS 每次启动后的首次转换均要求经系统文件夹选择器确认输出目录，使用当前 `NSOpenPanel` 对该进程授予的目录写入范围；后端继续用 `macos_security.rs` RAII 包住现有平台授权。`tauri-plugin-persisted-scope` 只保存 Tauri 的文件范围，不把路径伪装成跨启动的 macOS security-scoped bookmark。真实 MAS GUI prompt、重启后重新选择目录和转换结果仍需签名包实机验收。
...
- [x] **GitHub Actions 公共仓库成本策略**:仓库转 public 后只使用免费标准 runner(`ubuntu-24.04`/`ubuntu-24.04-arm`/`ubuntu-22.04`/`ubuntu-22.04-arm`/`windows-latest`/`macos-15`);CI 在 `main` push 与 PR 上自动跑 Linux 质量检查,fuzz 自动启用,Windows HEIC 与 Linux package smoke 在 main push 自动跑;Linux package smoke 与 `v*` tag release 统一使用 Ubuntu 22.04 兼容基线;macOS/Windows/updater workflow 仍手动触发但不再需要 paid confirmation;`pnpm run ci:cost:check` 静态防回退到 paid runner 或 schedule。
