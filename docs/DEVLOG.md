...
## 2026-09-01 — v0.2.24 格式披露与正式候选边界

- **格式能力披露**：将容易在窄窗口截断的引擎格式清单移到 Dropzone 的原生 `details` 披露区，摘要明确显示常见图片、HEIC 与 PDF；展开后按运行时 capability 分列导入/导出格式，并标记可选 HEIC 解码能力。375px E2E 覆盖键盘展开、焦点保持、44px 触控目标和无横向溢出。
- **状态栏收敛**：header 只报告“本地处理就绪”或“网页预览就绪”，不再把 codec provider 与完整格式列表挤进单行；中英文死键同步删除并由 i18n 对称门禁保护。
- **候选纪律**：`v0.2.24` 从同一不可变标签构建 Linux、macOS、Windows 直发包及 MAS/MSIX 商店候选。GitHub Release 只创建草稿；MAS 保持 `upload_build=false`，Microsoft Store 只生成未上传的 submission MSIX。全部签名、公证、staple、安装/转换 smoke 与校验和完成前不得公开。
...
- **全链路保护**：前端默认值、持久化迁移、Tauri serde 默认值和输出路径都有回归测试；SVG 保持原有豁免，不把矢量源字节数与栅格输出做无意义比较。
- **市场资料**：Snap 的打包描述和 Store listing 已统一，并准确说明“转换优先、压缩策略可选”，使 Linux 市场发布校验恢复通过。
